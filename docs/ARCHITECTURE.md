...
### 2.13 The library card — where your books live

The focus sheet's third block is not a question, it is an answer: the library
lives on this device. `focusAccountBlock()` in `js/settings.js` renders a
heading, a card and one paragraph of copy. There is no control, no state to
read and no module behind it.

> "It lives on this device, and nothing here needs an account to read. You can
> add one later to carry your shelf between phones — until you ask, nothing
> leaves."

**Honesty rules (binding).** The app never reports an account it cannot back,
never exposes a sign-in control that does nothing, and never implies data
leaves the device. There is no network call anywhere near this block and
nothing in it can produce one.

**Scope.** Nothing is uploaded, there is no server, and the README/COPYRIGHT
claims stand as written. A future sync provider **must** revise both — that is
a product decision with a paper trail, not a config change, and it would bring
its own UI rather than reviving a seam that shipped unreachable.
